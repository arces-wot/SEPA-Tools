...
public class ChatMonitor implements ISubscriptionHandler {
	...
	public ChatMonitor(int users, int messages) throws SEPAProtocolException, SEPAPropertiesException,
			SEPASecurityException, SEPABindingsException, InterruptedException {
		JSAPProvider jsapProvider = new JSAPProvider();
		client = new GenericClient(jsapProvider.getJsap(), jsapProvider.getSecurityManager(), this);

		totalMessages = users * (users - 1) * messages;

		client.subscribe("SENT", null, null, "sent",jsapProvider.getTimeout(),jsapProvider.getNRetry());
		client.subscribe("RECEIVED", null, null, "received",jsapProvider.getTimeout(),jsapProvider.getNRetry());
	}
